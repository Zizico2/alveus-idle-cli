use alveus_idle_cli::content::{can_interact, ItemId};
use alveus_idle_cli::components::TilePosition;
use alveus_idle_cli::interaction::{try_drop_item, try_feed_animal, try_give_item, PlayerSatchel};
...
fn test_try_feed_animal_consumes_matching_item() {
    ...
    assert!(
        try_feed_animal(&mut satchel, ItemId::TortoiseLeafyGreens).is_ok()
    );
...
fn test_try_feed_animal_rejects_wrong_item() {
    ...
    assert!(
        try_feed_animal(&mut satchel, ItemId::TortoiseLeafyGreens).is_err()
    );
    assert_eq!(satchel.item, Some(ItemId::ChickenGrains));
}
